...
public class StatusBarUtil {
    ...
    public static void setTranslucent(Activity activity, @IntRange(from = 0, to = 255) int statusBarAlpha, @IntRange(from = 0, to = 1) int mode) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.KITKAT) {
            return;
        }
        setTransparent(activity);
     ...
     * @param color    状态栏颜色
     * @param alpha    透明度
     * @param mode     状态栏字体颜色（0：LightMode，other：DarkMode）
     */
    public static void setStatusColor(Activity activity, @ColorInt int color, @IntRange(from = 0, to = 255) int alpha, int mode) {
     ...
     * @param mode     状态栏字体颜色（0：LightMode，other：DarkMode）
     */
    public static void setStatusColor(Activity activity, @ColorInt int color, int mode) {
        setColor(activity, color, mode);
        if (mode == 0) {
            setLightMode(activity);
        } else {
            setDarkMode(activity);
